...
if(!function_exists('bfi_get_home_path')){
		function bfi_get_home_path() {
...
if ( ! class_exists( 'BFI_Thumb_1_3' ) ) {
	...
	class BFI_Thumb_1_3 {
	    ...
	    public static function thumb( $url, $params = array(), $single = true ) {
		    ...
		    $dir = '' . $upload_dir . '/wpfuneralpress-cache';
	    ...
	    if (!is_dir($dir)) {
            ...
            mkdir($dir, 0777);
	       	     ...
	       	     $img_path = bfi_get_home_path() .  wp_make_link_relative( $url) ;
	        ...
	        $destfilename = "{$upload_dir}/uploads/wpfuneralpress-cache/{$dst_rel_path}-{$suffix}.{$ext}";

			// The urls generated have lower case extensions regardless of the original case
			$ext = strtolower( $ext );
	        $img_url = "{$upload_url}/uploads/wpfuneralpress-cache/{$dst_rel_path}-{$suffix}.{$ext}";

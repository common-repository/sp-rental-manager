...
add_action( 'wp_ajax_sp_rm_download_application','sp_rm_ajax_download_application' );
add_action( 'wp_ajax_sp_rm_download_applications',  'sp_rm_ajax_download_applications' );




	function sp_rm_ajax_download_application(){
			global $wpdb;
  ...
  if(class_exists('sprm_FormBuilderUser')){
    	...
    	$sprm_FormBuilderUser = new sprm_FormBuilderUser;
...
echo '</body>';
      die();
		...
		function sp_rm_ajax_download_applications(){
			global $wpdb, $sprm_FormBuilderUser;
...
<th>'.__("Action","sp-rm").'</th>
	...
	<a class="button" style="margin-left:20px" href="'.admin_url('admin-ajax.php?action=sp_rm_download_application&id='.$r[$i]['id'].'').'" target="_blank">'.__("Print","sp-rm").'</a>
	</td>
		</tr>';
		
	}
	
return  apply_filters('sp_rm/admin/applications/list',$content);
}

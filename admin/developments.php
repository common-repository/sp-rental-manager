...
<tr>
	...
	foreach( $features as $key => $value){
		...
		$features_content.= '<div id="sp_rm_feature_main" class="sp_rm_feature">Feature Name <input type="text" name="features[]" value="'.stripslashes($features[$i]).'"> Feature Value <input type="text" name="features_value[]" value="'.stripslashes($features_values[$i]).'"></div>';
			$i++;
	}
	}
	$features_content.= '<div id="sp_rm_feature_end"></div>
	</td>
	</tr>';
...
<th width="450">'.__("Action","sp-rm").'</th>
	...
	$bg= ' style="background-color:#ffe2e2" ';
		...
		<td>'.$r[$i]['id'].'</td>';
		  ...
		  if(class_exists('sprm_FormBuilderPremium')){
			$sprm_FormBuilderPremium = new sprm_FormBuilderPremium;
			 echo '<td>'.$sprm_FormBuilderPremium->applicationCount($r[$i]['id']).'</td>'; 
		  }else{
	...
	<a class="button" style="margin-left:20px" href="admin.php?page=sp-rm-developments&function=manage-listing&id='.$r[$i]['id'].'">'.__("View","sp-rm").'</a> 
		<a class="button" style="margin-left:20px" href="'.admin_url('admin-ajax.php?action=sp_rm_download_applications&id='.$r[$i]['id'].'').'" target="_blank">'.__("Download Applications","sp-rm").'</a>
	'.	$publish .'
	</td>
		</tr>';
